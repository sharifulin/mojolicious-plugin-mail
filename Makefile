# This Makefile is for the Mojolicious::Plugin::Mail extension to perl.
#
...
#     EXE_FILES => []
#     INSTALLDIRS => q[site]
#     NAME => q[Mojolicious::Plugin::Mail]
#     PL_FILES => {  }
#     PREREQ_PM => { Test::More=>q[0], MIME::EncWords=>q[0], MIME::Lite=>q[3.027], Encode=>q[0], Mojolicious=>q[0.999937] }
#     VERSION_FROM => q[lib/Mojolicious/Plugin/Mail.pm]
...
NAME_SYM = Mojolicious_Plugin_Mail
VERSION = 0.81
...
MAKE = make
FULLEXT = Mojolicious/Plugin/Mail
BASEEXT = Mail
...
PM_TO_BLIB = README.pod \
	...
	lib/Mojolicious/Plugin/Mail.pm \
	blib/lib/Mojolicious/Plugin/Mail.pm
...
DISTNAME = Mojolicious-Plugin-Mail
DISTVNAME = Mojolicious-Plugin-Mail-0.81
...
manifypods : pure_all  \
	...
	$(NOECHO) $(POD2MAN) --section=3 --perm_rw=$(PERM_RW) \
	  ...
	  lib/Mojolicious/Plugin/Mail.pm $(INST_MAN3DIR)/Mojolicious::Plugin::Mail.$(MAN3EXT) 
...
metafile : create_distdir
	...
	$(NOECHO) $(ECHO) 'name:                Mojolicious-Plugin-Mail' >> META_new.yml
	$(NOECHO) $(ECHO) 'version:             0.81' >> META_new.yml
	...
	$(NOECHO) $(ECHO) '    MIME::Lite:                    3.027' >> META_new.yml
	$(NOECHO) $(ECHO) '    Mojolicious:                   0.999937' >> META_new.yml
...
ppd :
	$(NOECHO) $(ECHO) '<SOFTPKG NAME="$(DISTNAME)" VERSION="0,81,0,0">' > $(DISTNAME).ppd
	$(NOECHO) $(ECHO) '    <TITLE>$(DISTNAME)</TITLE>' >> $(DISTNAME).ppd
	$(NOECHO) $(ECHO) '    <ABSTRACT></ABSTRACT>' >> $(DISTNAME).ppd
	$(NOECHO) $(ECHO) '    <AUTHOR></AUTHOR>' >> $(DISTNAME).ppd
	$(NOECHO) $(ECHO) '    <IMPLEMENTATION>' >> $(DISTNAME).ppd
	$(NOECHO) $(ECHO) '        <DEPENDENCY NAME="Encode" VERSION="0,0,0,0" />' >> $(DISTNAME).ppd
	$(NOECHO) $(ECHO) '        <DEPENDENCY NAME="MIME-EncWords" VERSION="0,0,0,0" />' >> $(DISTNAME).ppd
	$(NOECHO) $(ECHO) '        <DEPENDENCY NAME="MIME-Lite" VERSION="3,027,0,0" />' >> $(DISTNAME).ppd
	$(NOECHO) $(ECHO) '        <DEPENDENCY NAME="Mojolicious" VERSION="0,999937,0,0" />' >> $(DISTNAME).ppd
...
pm_to_blib : $(TO_INST_PM)
	$(NOECHO) $(ABSPERLRUN) -MExtUtils::Install -e 'pm_to_blib({@ARGV}, '\''$(INST_LIB)/auto'\'', '\''$(PM_FILTER)'\'')' -- \
	  ...
	  lib/Mojolicious/Plugin/Mail.pm blib/lib/Mojolicious/Plugin/Mail.pm 
